...
def unstructured_prune(path='./cifar_net.pth', prune_rate=0, accuracy_test=True, results_path='./prune_results.txt'):
    ...
    with torch.no_grad():
        for name, param in model.named_parameters():
            ...
            # print("alpha:", alpha)
            sorted_param = sorted(torch.abs(param.view(-1)))
            max_idx = int(len(list(param.view(-1))) * alpha)
            max_val = sorted_param[max_idx - 1]

            new_param = torch.where(torch.abs(param) >= max_val, param, torch.zeros(param.size(), dtype=torch.float32).cuda())

            param.copy_(new_param).cuda()
